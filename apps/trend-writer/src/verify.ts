...
3. 양식 준수 — # 제목, '💡 핵심 요약' 문단, '## 1.'~'## 6.' 6단 헤딩이 있는가.
4. 슬롭 없음 — 본문(6단 구조 영역)에 이중 피동·기계적 병렬·"결론적으로/시사하는 바가 크다" 등 AI 티가 과하지 않은가.
5. 날조 없음 — 존재하지 않는 자체 프로젝트·제품·사내 시스템 이름(예: 'C-Market 어드민')을 지어내거나, 근거에 없는 1인칭 실무 경험·구체 장애/수치("우리 서비스에서 429 폭주를 겪었습니다" 등)를 실제인 것처럼 서술하지 않았는가. 하나라도 있으면 pass=false.
